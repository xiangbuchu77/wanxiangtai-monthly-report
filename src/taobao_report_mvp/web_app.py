...
class ReportAppHandler(BaseHTTPRequestHandler):
    ...
    def handle_generate(self) -> dict[str, Any]:
        ...
        uploads = save_uploads(form, "reports", upload_dir)
    ...
    def run_qclaw_request(self, text: str, files: list[Path], settings: dict[str, Any]) -> dict[str, Any]:
        ...
        files = expand_archives(files, OUTPUT_ROOT / "qclaw_unzipped" / datetime.now().strftime("%Y%m%d_%H%M%S"), QCLAW_EXTRACTABLE_SUFFIXES)
        report_files = [path.resolve() for path in files if path.suffix.lower() in QCLAW_REPORT_SUFFIXES and path.exists()]
        document_files = [path.resolve() for path in files if path.suffix.lower() not in QCLAW_REPORT_SUFFIXES and path.exists()]
        ...
        if wants_report and report_files:
            ...
            for item_period in periods:
                ...
                store_last_report(session_key, result, item_report_files, item_period, target)
            ...
            response.setdefault("audit", {})["daily_store_cache"] = cache_summary
            return response
        if document_files:
            return {
                "type": "needs_input",
...
def save_uploads(
    ...
    for field in fields:
        ...
        if suffix == ".zip":
            saved.extend(expand_zip_archive(destination, target_dir / f"{destination.stem}_unzipped", allowed_suffixes or SUPPORTED_SUFFIXES))
    return saved
...
def archive_daily_report_files(report_files: list[Path], preferred_store: str = "") -> DailyArchiveResult:
    files = unique_paths(report_files)
    if not files:
...
def write_daily_cache_index(stores: dict[str, list[Path]]) -> None:
    for store, paths in stores.items():
        ...
        all_paths = unique_paths([*existing_paths, *paths])
        payload = {
            "store_name": store,
...
def daily_cached_files_for_store(store_name: str) -> list[Path]:
    ...
    if metadata.exists():
        try:
            payload = json.loads(metadata.read_text(encoding="utf-8"))
            return unique_paths([Path(path) for path in payload.get("files", [])])
        except Exception:
            pass
    return unique_paths([path for path in cache_dir.iterdir() if path.suffix.lower() in QCLAW_REPORT_SUFFIXES])
...
def resolve_daily_store_files_for_generation(
    ...
    if selected_store:
        ...
        return (unique_paths([*cached, *report_files]) or report_files, None)
