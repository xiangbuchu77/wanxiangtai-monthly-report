...
from .archive_utils import extract_zip_archive
...
APP_ROOT = Path(os.environ.get("WXT_APP_ROOT", ".")).expanduser().resolve()
PACKAGE_DIR = Path(__file__).resolve().parent


def template_candidates(filename: str) -> list[Path]:
    """Find templates in both the shared workspace and standalone Skill layouts."""
    candidates = [
        APP_ROOT / "templates" / filename,
        APP_ROOT / "skills" / "wanxiangtai-monthly-report" / "templates" / filename,
        Path.cwd() / "templates" / filename,
    ]
    for ancestor in PACKAGE_DIR.parents:
        candidates.extend(
            [
                ancestor / "templates" / filename,
                ancestor / "skills" / "wanxiangtai-monthly-report" / "templates" / filename,
                ancestor / "assets" / "service" / "templates" / filename,
            ]
        )
    unique: list[Path] = []
    seen: set[Path] = set()
...
DEFAULT_TEMPLATE_CANDIDATES = template_candidates("万相台月报模板.xlsx")
...
def run_workflow(
    ...
    ad_inputs = [item.path for item in csv_classified if item.kind in {"plan_report", "marketing_scene_report"}]
    ...
    cleanup_workflow_artifacts(outputs, ai_json_paths, packet_paths)
...
def cleanup_workflow_artifacts(outputs: WorkflowOutputs, ai_json_paths: list[str], packet_paths: list[str]) -> None:
    """The delivery contract exposes only final period reports, never workbooks used during calculation."""
    for path in [
        outputs.standard_total,
        outputs.ad_semifinished,
        outputs.validation_report,
        *(Path(item) for item in ai_json_paths),
        *(Path(item) for item in packet_paths),
    ]:
        try:
...
def expand_zip_inputs(paths: list[Path], extract_root: Path) -> list[Path]:
    ...
    for path in paths:
        ...
        if source.suffix.lower() == ".zip":
            expanded.extend(extract_zip_archive(source, extract_root / source.stem, {".xlsx", ".xls", ".csv"}))
        else:
...
def template_for_period(template: Path, period: str) -> Path:
    ...
    candidates = [template.parent / expected_name, *template_candidates(expected_name)]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    return template
